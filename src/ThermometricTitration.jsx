...
const ThermometricTitration = () => {
  ...
  // Interactive Slider Crosshair States
  ...
  const generateProblem = (forcedMode = null) => {
    ...
    const volAcid = 25.0; 
    // Endpoint can be a decimal matching 0.1 graduation limits when zoomed
    ...
    if (selection === 'read_graph') {
      newProb = {
        ...
        text: <>A student titrated 25.0 cm³ of hydrochloric acid against sodium hydroxide. Click/tap and drag the <b>vertical cursor line</b> to align with the intersection vertex, then zoom in to determine the exact neutralisation endpoint volume.</>,
  ...
  const handleMove = (clientX) => {
    ...
    const computedVol = parseFloat((pct * 50).toFixed(1));
  ...
  const checkAnswer = () => {
    ...
    if (mode === 'read_graph') {
      ...
      if (Math.abs(userNum - targetNum) < 0.15) {
        ...
        setFeedback({ message: `Incorrect endpoint reading. Zoom in and use the 0.1 cm³ subdivisions to locate the line intersection.`, status: 'error' });
      }
      return;
    }
    ...
    // Checking 3 s.f. specific string format properties via feedback regex rules
    const rawClean = rawInput.replace('-', '');
    const sFigs = rawClean.replace('.', '').replace(/^0+/, '');
    if (sFigs.length !== 3 && !rawClean.endsWith('.0')) {
      setFeedback({
        message: "Precision Error: Your calculation is close, but check your rounding rules. The question explicitly demands your answer to exactly 3 significant figures.",
        status: 'error'
      });
      return;
    }
  ...
  // Base coordinates mapping generators
  const xStart = 40; const xEnd = 280;
  const yTop = 20; const yBottom = 130;
  ...
  // High resolution zoom range viewport configurations
  const zoomMinVol = Math.max(0, problem.graphData.endpointVol - 3);
  const zoomMaxVol = Math.min(50, problem.graphData.endpointVol + 3);

  // Array generator loop helper for 0.1 sub-divisions
  const getZoomTicks = () => {
    let ticks = [];
  ...
  return (
    <div className="applet-container" style={{ textTransform: 'none' }}>
      ...
      {/* --- ACTIVE INTERACTIVE GRAPH FRAME --- */}
      {mode === 'read_graph' && (
        <div className="w-full max-w-md mx-auto my-4 flex flex-col items-center gap-3">
          <div 
          ...
          >
            ...
            >
              ...
              {/* Major X Axis Label Ticks */}
              {[0, 10, 20, 30, 40, 50].map((v) => (
                <g key={v}>
                  <line x1={getX(v)} y1={yBottom} x2={getX(v)} y2={yBottom + 4} stroke="#94a3b8" />
                  <text x={getX(v)} y={yBottom + 14} textAnchor="middle" className="text-[9px] font-mono font-bold fill-slate-400">{v}</text>
                </g>
              ))}
              <text x="160" y="158" textAnchor="middle" className="text-[9px] font-black fill-slate-400 uppercase tracking-wider">Volume of NaOH added (cm³)</text>
              ...
              {/* Interactive Sliding User Target Line */}
          ...
          <div className="flex w-full justify-between items-center px-1">
            <div className="text-xs font-bold text-slate-500">
              Selected: <span className="font-mono font-black text-amber-500 bg-amber-50 px-2 py-0.5 border border-amber-200 rounded">{userVol.toFixed(1)} cm³</span>
            </div>
            <button 
              type="button" 
              onClick={() => setIsZoomed(!isZoomed)}
              className={`px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider shadow-sm transition-all border ${
                isZoomed ? 'bg-amber-500 text-white border-amber-500' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
              }`}
            >
              {isZoomed ? "Close Zoom View 🔍" : "Open Zoom View 🔍"}
            </button>
          </div>

          {/* --- HIGH PRECISION 0.1 CM³ SUBDIVISION ZOOM CARD --- */}
          {isZoomed && (
            <div className="w-full bg-slate-900 text-white rounded-2xl p-4 border border-slate-800 shadow-inner flex flex-col items-center animate-in fade-in zoom-in-95 duration-100">
              <span className="text-[9px] font-black tracking-widest uppercase text-amber-400 mb-3">High Precision Close-up Axis (0.1 cm³ steps)</span>
              
              <div className="w-full overflow-x-auto py-2">
                <div className="min-w-[480px] h-14 relative border-b border-slate-700">
                  {getZoomTicks().map((v) => {
                    // Normalize position percentages inside the zoom slider strip
                    ...
                    return (
                      <div key={v} className="absolute top-0 transform -translate-x-1/2 flex flex-col items-center" style={{ left: leftPos }}>
                        <div className={`w-[1px] bg-slate-600 ${isInt ? 'h-5 bg-slate-400' : 'h-2.5'}`} />
                        {isInt && <span className="text-[10px] font-mono font-bold text-slate-400 mt-1">{v}.0</span>}
                      </div>
                    );
                  })}

                  {/* Target Vertex True Value Flag Line */}
                  <div 
                    className="absolute top-0 w-[2px] h-7 bg-emerald-500 z-10 border-r border-emerald-400/30"
                    style={{ left: `${((problem.graphData.endpointVol - zoomMinVol) / (zoomMaxVol - zoomMinVol)) * 100}%` }}
                  >
                    <span className="absolute -top-3.5 left-1/2 transform -translate-x-1/2 text-[8px] bg-emerald-600 text-white px-1 rounded font-black">INTERSECTION</span>
                  </div>

                  {/* Current Position Slider Tracker cursor */}
                  <div 
                    className="absolute top-0 w-0.5 h-8 bg-amber-500 z-20 shadow-glow"
                    style={{ left: `${((userVol - zoomMinVol) / (zoomMaxVol - zoomMinVol)) * 100}%` }}
                  >
                    <div className="w-2 h-2 bg-amber-500 rounded-full absolute -bottom-1 left-1/2 transform -translate-x-1/2" />
                  </div>
                </div>
              </div>
            </div>

...
const EmpiricalFormula = () => {
  ...
  const generateProblem = (forcedMode = null) => {
    ...
    let newProb = {};
    const k = parseFloat((0.015 + Math.random() * 0.025).toFixed(4)); // Scaling factor for mass randomization

    if (targetMode === 'empirical_pct') {
      const pool = [
        { name: 'hydrocarbon gas', elements: 'Carbon and Hydrogen', pctC: 85.63, pctH: 14.37, ans: 'CH2' },
        { name: 'toxic oxide of nitrogen', elements: 'Nitrogen and Oxygen', pctN: 30.43, pctO: 69.57, ans: 'NO2' },
        { name: 'phosphorus oxide derivative', elements: 'Phosphorus and Oxygen', pctP: 43.64, pctO: 56.36, ans: 'P2O5' },
        { name: 'liquid hydrocarbon', elements: 'Carbon and Hydrogen', pctC: 92.26, pctH: 7.74, ans: 'CH' },
        { name: 'solvent compound', elements: 'Carbon, Hydrogen and Chlorine', pctC: 24.26, pctH: 4.07, pctCl: 71.67, ans: 'CH2Cl' }
      ];
      const selected = pool[Math.floor(Math.random() * pool.length)];
      
      const dataText = selected.pctCl 
        ? `${selected.pctC}% Carbon, ${selected.pctH}% Hydrogen, and ${selected.pctCl}% Chlorine`
        : `${selected.pctC}% Carbon and ${selected.pctH}% Hydrogen`;

      newProb = {
        title: "Empirical Formula via Percentage Composition",
        text: <>Chemical analysis of an unknown {selected.name} confirms it contains <b>{dataText}</b> by mass.</>,
        question: "Deduce the simplest empirical formula for this compound.",
        placeholder: "e.g. P2O5",
      ...
      const pool = [
        { emp: 'CH2O', mol: 'C6H12O6', mr: 180, name: 'monosaccharide carbohydrate' },
        { emp: 'CH2', mol: 'C4H8', mr: 56, name: 'gaseous alkene isomer' },
        { emp: 'C2H5', mol: 'C4H10', mr: 58, name: 'liquefied alkane fuel cell' },
        { emp: 'CH', mol: 'C6H6', mr: 78, name: 'aromatic hydrocarbon solvent' },
        { emp: 'CH2Cl', mol: 'C2H4Cl2', mr: 99, name: 'halogenated organic backbone' }
      ];
      const selected = pool[Math.floor(Math.random() * pool.length)];
      ...
      const pool = [
        { salt: 'copper(II) sulfate', formula: 'CuSO4', anhydrousMr: 159.62, waterRatio: 5 },
        { salt: 'magnesium sulfate', formula: 'MgSO4', anhydrousMr: 120.37, waterRatio: 7 },
        { salt: 'barium chloride', formula: 'BaCl2', anhydrousMr: 208.23, waterRatio: 2 },
        { salt: 'calcium chloride', formula: 'CaCl2', anhydrousMr: 110.98, waterRatio: 2 }
      ];
      const selected = pool[Math.floor(Math.random() * pool.length)];
      ...
      newProb = {
        ...
        text: displayOption 
          ? <>A student heats a <b>{massTotal} g</b> sample of hydrated {selected.salt} ( {selected.formula}·<i>x</i>H<sub>2</sub>O ) strongly inside a crucible until it reaches a constant mass. The remaining anhydrous salt residue weights exactly <b>{massAnhydrous} g</b>.</>
  ...
  const checkAnswer = () => {
    ...
    // Convert premium formatting subscripts back to baseline alphanumeric formats for checking comparisons
  ...
  return (
    <div className="applet-container" style={{ textTransform: 'none' }}>
      ...
      {/* --- CELL CONTAINER LAYOUT GRID INPUT PANELS --- */}
